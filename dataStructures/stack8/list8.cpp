...
void PutList(List *L, BaseTypeList8 E)
{
	if (L->N < L->SizeMem)
	{
		void *tmp = new char[sizeof(char) * 20]; // Выделение памяти под информационную часть элемента
		std::memcpy(tmp, E, sizeof(char) * 20);	 // Копирование информационной части элемента
												 // Запись указателя на выделенную память в массив указателей
...
// std::memcpy(tmp, E, sizeof(char) * 20);
// *(int *)tmp = *(int *)E;
...
void GetList(List *L, BaseTypeList8 *E)
{
	if (L->N > 0)
	{
		...
		std::memcpy((void *)*(int *)E, (*L->PMemList)[--L->ptr], sizeof(char) * 20);
...
void ReadList(List *L, BaseTypeList8 *E)
{
	if (L->N > 0)
	{
		std::memcpy((void *)*(int *)E, (*L->PMemList)[L->ptr - 1], sizeof(char) * 20);
...
void CopyList(List *L1, List *L2)
{
	...
	{
		void *tmp = new char[sizeof(BaseTypeList8)]; // Выделение памяти под информационную часть элемента
